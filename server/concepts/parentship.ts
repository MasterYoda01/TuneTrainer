...
export default class ParentConcept {
  ...
  /**
   * removes all records of the child having a parent
   * @param child
   */
  async orphan(child: ObjectId) {
    await this.parentships.deleteMany({ child });
  }
  ...
  async getAllParents(child: ObjectId): Promise<ObjectId[]> {
    return (await this.parentships.readMany({ child })).map(({ parent }) => parent);
  }
}
